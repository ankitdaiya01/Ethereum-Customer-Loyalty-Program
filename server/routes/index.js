...
let accounts = null;
if(1){
    accounts = ["0x05422Ad046387CFE579634D1a30BbF3Db0eF5859"]
}
console.log("0x806610FA77Db85D94EF54eC68922D8bbaC322F9C");
const getInstance = () => {
    return new web3.eth.Contract(
        JSON.parse(LoyaltyFactory.interface),
        "0x4d9441c173917B1d5880f96a58164C20339AEaEd"
    );
}
...
apiRoutes.post('/authenticate', function (req, res) {
    if (req.body.name === 'root' && req.body.password === 'root') {
        ...
        var token = jwt.sign(payload, 'tester', {
...
apiRoutes.use(function (req, res, next) {
    ...
    console.log(token);
    if (token) {
        jwt.verify(token, 'tester', function (err, decoded) {
            if (err) {
...
apiRoutes.get('/getCompamies', async function (req, res) {
    ...
    res.json({
        success: true,
        addr: 0x4d9441c173917B1d5880f96a58164C20339AEaEd,
        companies: companies
    });
